  ...
  return (
    <div id="ops-intelligence-center" className="bg-brand-bg border border-brand-border rounded-xl p-4 flex flex-col h-full shadow-2xl overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden mt-3 relative">
        ...
        {/* ── TAB: SECURITY ── */}
        {activeTab === 'security' && (
          <div className="flex flex-col h-full bg-[#050505]/40 rounded border border-brand-border p-3 space-y-3 overflow-y-auto">
            <div className="flex items-center justify-between border-b border-brand-border pb-2">
              <div>
                <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider">Resultado de la Auditoría</h3>
                <p className="text-[10px] text-brand-text-muted mt-0.5">Políticas de Hardening e integridad del Sistema</p>
              </div>
              <button onClick={onRunAuditScan}
                className="px-3 py-1 bg-brand-bar border border-brand-border hover:bg-[#333] text-zinc-350 text-xs font-semibold rounded cursor-pointer">
                Escanear de Nuevo
              </button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {securityAssets.map((asset) => {
                const badge = { secure: 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400', warning: 'bg-amber-500/10 border-amber-500/30 text-amber-500', danger: 'bg-rose-500/10 border-rose-500/30 text-rose-500' };
                return (
                  <div key={asset.id} className="bg-brand-panel border border-brand-border p-3 rounded">
                    <div className="flex items-start justify-between gap-1">
                      ...
                      <span className={'px-2 py-0.5 text-[9px] uppercase font-bold rounded border ' + (badge[asset.status] || '')}>{asset.status}</span>
                    </div>
                    <p className="text-[11px] text-brand-text-muted mt-2 leading-relaxed">{asset.details}</p>
                  </div>
                );
              })}
            </div>
